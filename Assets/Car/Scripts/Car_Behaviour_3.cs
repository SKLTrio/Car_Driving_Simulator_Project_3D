...
public class Car_Behaviour_3 : MonoBehaviour
{
    ...
    void Update()
    {
        Acceleration();
        Deceleration();
        Vector2 turnInput = Car_Input_Controls.Gameplay.Turn.ReadValue<Vector2>();
        Turning(turnInput);
    }
    ...
    {
        Vector2 Move_Input = Car_Input_Controls.Gameplay.Decelerate.ReadValue<Vector2>();
        float decelerate_Input = Move_Input.y;

        if (decelerate_Input > 0)
        {
    ...
    public void Turning(Vector2 turnInput)
    {
        float Steer_Input = turnInput.x * Steer_Speed;
